...
class postBuildServer(configFileIni):
    ...
    def mainExecCMD(self, command, param=None , param2=None):
        ...
        cmdExecInstallEnv         = """ ssh -o "StrictHostKeyChecking=no" -o "ConnectTimeout=2" -i "%s" "%s@%s" "sudo bash /var/%s/build.sh --docker_env yes --docker_compose_up yes" """ % (self.ssh_key_path, self.user_name, self.server_ip, param)
        ...
        cmdInitSwarmManager       = """ ssh -o "StrictHostKeyChecking=no" -o "ConnectTimeout=2" -i "%s" "%s@%s" "sudo bash /var/%s/build.sh  --init_swarm_manager yes" """ % (self.ssh_key_path, self.user_name, self.server_ip, param)
        cmdTochToken              = """ touch %s """  % (param)
        cmdConnectWorkerToManager = """ ssh -o "StrictHostKeyChecking=no" -o "ConnectTimeout=2" -i "%s" "%s@%s" "sudo bash /var/%s/build.sh --docker_env yes --worker_connect_to_manager %s" """ % (self.ssh_key_path, self.user_name, self.server_ip,param,param2)
